...
export const registerUser = async(req,res)=>{
    try {
        ...
        if(existUserName){
            return res.status(400).json({message:"User with given user name already exists"})
        }
...
export const loginUser = async(req,res)=>{
    try {
        ...
        const token = await jwt.sign({id:existUser._id,name:existUser.name},process.env.SECRET_KEY, {expiresIn:'7h'});
